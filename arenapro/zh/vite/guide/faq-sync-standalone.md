...
- 你已经在浏览器登录了神岛账号；
- 项目根目录下的 `.env` 中，已经配置好要同步的目标地图 ID（`VITE_DAO3_MAP_ID`）等信息。
...
## 3. 什么时候应该先跑 `apc login`，什么时候跑 `apc resource`？
...
3. **日常开发 & 构建上传：**
   ...
   - 开发阶段：`apc dev`（或 `npm run dev`）
   - 发布前：`apc build`（或 `npm run build`）

## 4. 常见问题速查

- **Q：换了账号 / Token 过期了，怎么处理？**

  - A：重新执行一次 `apc login`，让脚手架帮你重新写入最新的 Token / UA。
